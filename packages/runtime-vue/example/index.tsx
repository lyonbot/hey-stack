...
import { defineScopeComponent, defineScopeVariable, ScopeFor } from '../src/index.js'
import { mixinHighlightChanges } from './utils.js'

const App = defineScopeComponent((ctx) => {
  ...
  return () => (
    <div>
      ...
      <ul>
        ...
        <li>
          <button onClick={() => ctx.todos.push({ id: (ctx.todos.at(-1)?.id ?? -1) + 1, text: 'New todo' })}>Add todo</button>
        </li>
      </ul>
...
const TodoItem = defineScopeComponent((ctx: { todos: any[], todo: any, index: number }) => {
  return () => (
    <li>
      {ctx.todo.id}
      <input value={ctx.todo.text} onChange={e => ctx.todo.text = (e.target as HTMLInputElement).value} />
